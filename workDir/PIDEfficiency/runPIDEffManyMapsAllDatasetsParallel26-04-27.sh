...
OUTPUT_DIR="output/IdasMapsPromptNonPSampa" #! ATENTION: Must be changed by hand!


MC_LIST=(LHC25b16 LHC25b17) #! ATENTION: Must be changed by hand!
...
do
    ...
    AODPath="$DATASETS_DIR/$i/input.txt"

    if [[ "$AODPath" == *.txt ]]; then   # listas de AOD precisam começar com @
        AO2D_LIST+=("@${AODPath}")
    else
        AO2D_LIST+=("${AODPath}")
    fi
